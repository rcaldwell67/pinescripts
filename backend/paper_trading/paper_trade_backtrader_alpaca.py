...
# ── Logging setup ─────────────────────────────────────────────────────────────
LOG_PATH = REPO_ROOT / "backend" / "paper_trade_error.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_paper_to_db(symbol: str, version: str, trades, df, *, force_reset: bool = False) -> None:
    try:
        ...
        from strategy_generator.symbol_id_lookup import get_symbol_id
            ...
            if not symbol_id:
                logger.error(f"[ERROR] Could not find symbol_id for symbol {symbol}")
            return
        conn = get_db_conn()
        cur = conn.cursor()
        ...
        if force_reset:
            cur.execute(
                 "DELETE FROM trades WHERE symbol_id = %s AND version = %s AND mode = 'paper'",
                 (symbol_id, version),
            )
            last_exit_time = None
        else:
            cur.execute(
                 "SELECT MAX(exit_time) FROM trades WHERE symbol_id = %s AND version = %s AND mode = 'paper'",
                 (symbol_id, version),
            )
            row = cur.fetchone()
