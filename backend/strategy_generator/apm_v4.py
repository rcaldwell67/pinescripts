...
def apm_v4_signals(df, side: str = "short", params: dict[str, Any] | None = None):
    cfg = params or get_v4_params()
...
def apm_v4_latest_bar_analysis(df, side: str = "short", params: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg = params or get_v4_params()
    signal = cfg.get("signal", {})
    if side == "long" and not bool(signal.get("enable_longs", False)):
        return {
            ...
            "detail": "long side disabled in v4 params",
            "latest_bar_ts": None,
        }
    if side == "short" and not bool(signal.get("enable_shorts", True)):
        return {
            ...
            "passed_stage": "start",
            "failed_stage": "disabled",
            "detail": "short side disabled in v4 params",
            "latest_bar_ts": None,
        }
    return apm_v1_latest_bar_analysis(df, side=side, params=cfg)


def apm_v4_latest_bar_exit_analysis(df, side: str = "short", params: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg = params or get_v4_params()
