...
def parse_args():
    ...
    parser.add_argument("--process-all-chunks", action="store_true", help="Process all Stage 1 passing chunks for Stage 2")
    parser.add_argument("--process-all-stage1-chunks", action="store_true", help="Process all Stage 1 chunks before any Stage 2 processing")
...
if __name__ == "__main__":
    ...
    # Check if Stage 1 CSV for this chunk already exists and has rows
    ...
    if not skip_stage1:
        ...
        param_grid_all = [t for t in itertools.product(*grid.values()) if is_valid_combination(t)]
        total = len(param_grid_all)
        print(f"Total valid parameter combinations after filtering: {total}")
        ...
        max_chunk_rows = MAX_CHUNK_BYTES // EST_ROW_SIZE
        auto_num_chunks = (total + max_chunk_rows - 1) // max_chunk_rows
        print(f"Auto-calculated number of Stage 1 chunks: {auto_num_chunks}")
...
if __name__ == "__main__":
    ...
    def process_stage2_for_chunk(chunk_idx, chunk_file, num_chunks):
        ...
        if effective_num_chunks2 > 1:
            ...
            start2 = chunk_idx * chunk_size2
            end2 = min(start2 + chunk_size2, stage2_total)
            stage2_param_iter = passing_stage1[start2:end2]
            print(f"Stage 2: Evaluating chunk {chunk_idx+1}/{effective_num_chunks2}: {len(stage2_param_iter)} of {stage2_total} parameter sets...")
        else:
        ...
        with multiprocessing.Pool(processes=max_workers, initializer=stage2_init, initargs=(df,)) as pool:
            for result in pool.imap_unordered(stage2_worker, stage2_param_iter):
                ...
                if completed2 % save_every == 0:
                    tmp_csv2 = f"stage2_partial_{chunk_idx+1}_of_{num_chunks}.csv"
    ...
    import glob
    import re
    files = sorted(glob.glob("stage1_passing_params_chunk*_of_*.csv"))
    chunk_pattern = re.compile(r"stage1_passing_params_chunk(\d+)_of_(\d+)\.csv")
    found_passing = False
    for f in files:
        m = chunk_pattern.match(os.path.basename(f))
        if not m:
            continue
        ...
        num_chunks = int(m.group(2))
        print(f"\nProcessing Stage 2 for chunk {chunk_idx+1} of {num_chunks} ({f})...")
        process_stage2_for_chunk(chunk_idx, f, num_chunks)
