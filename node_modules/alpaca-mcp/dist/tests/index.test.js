...
describe('request', () => {
    ...
    it('throws if API keys are missing', async () => {
        await expect(request({ path: '/test' })).rejects.toThrow('Alpaca credentials not configured');
    });
    it('performs a successful GET request', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://api//endpoint', { method: 'GET', headers: { 'APCA-API-KEY-ID': 'key', 'APCA-API-SECRET-KEY': 'secret' } });
    ...
    it('throws with status and error body on failure', async () => {
        ...
        fetch.mockResolvedValue(mockRes);
        await expect(request({ path: '/e' })).rejects.toThrow('404 Not Found - {"error":"fail"}');
    });
});
describe('getAssets', () => {
    beforeEach(() => {
    ...
    it('fetches tradable assets', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://broker//v1/assets?status=active&asset_class=us_equity', { method: 'GET', headers: { 'APCA-API-KEY-ID': 'key', 'APCA-API-SECRET-KEY': 'secret' } });
...
describe('getStockBars', () => {
    ...
    it('fetches stock bars and returns aggregated bars', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://api//v2/stocks/bars?timeframe=1Day&limit=10000&start=2021-01-01&end=2021-01-02&symbols=A%2CB', { method: 'GET', headers: { 'APCA-API-KEY-ID': 'key', 'APCA-API-SECRET-KEY': 'secret' } });
...
describe('getMarketDays', () => {
    ...
    it('fetches market days', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://api//v2/calendar?start=2021-01-01&end=2021-01-02', { method: 'GET', headers: { 'APCA-API-KEY-ID': 'key', 'APCA-API-SECRET-KEY': 'secret' } });
...
describe('getNews', () => {
    ...
    it('fetches news articles', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://api//v1beta1/news?sort=desc&start=2021-01-01&end=2021-01-02&symbols=A%2CB&include_content=true', { method: 'GET', headers: { 'APCA-API-KEY-ID': 'key', 'APCA-API-SECRET-KEY': 'secret' } });
