"""Regenerate aligned v2 backtest/paper datasets in tradingcopilot.db.

This script runs the same v2 backtest simulation once per symbol and writes
...
def _backup_db(db_path: Path) -> Path:
    ...
    backup = db_path.with_name(f"{db_path.stem}.pre_align_v2_{stamp}{db_path.suffix}")
...
def _build_rows(symbol: str, trades, df) -> list[tuple]:
    ...
    for _, trade in trades.iterrows():
        ...
        side = str(trade.get("side", "short") or "short").lower().strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Regenerate aligned v2 backtest/paper datasets.")
    ...
    try:
        ...
        conn.commit()
        print("\nAligned v2 backtest/paper datasets regenerated.")
        return 0
