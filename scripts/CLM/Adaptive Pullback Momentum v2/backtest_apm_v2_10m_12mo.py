# ─────────────────────────────────────────────────────────────────────────────
# APM v2.0 — CLM 10m  ·  12-Month Backtest  (Alpaca IEX)
# Mirrors "Adaptive Pullback Momentum v2.0" Pine script parameters.
# Shorts-only (CLM sub-15m longs: WR too low historically).
#
...
print(f"\n{'='*60}")
print(f"APM v2.0 · 10m · CLM · 12-Month Backtest")
print(f"{'='*60}")
...
out_csv = _dir / "apm_v2_12mo_trades_clm_10m.csv"
...
ax.set_title(f"APM v2.0 · CLM 10m · 12-mo Equity  {net:+.2f}%  |  {len(tdf)}T  WR={wr:.0f}%  PF={pf:.2f}  DD={max_dd:.1f}%")
ax.set_ylabel("Equity ($)"); ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
ax.grid(alpha=0.2); plt.tight_layout()
out_png = _dir / "apm_v2_12mo_equity_clm_10m.png"
