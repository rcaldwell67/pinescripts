"""
APM v2.2  — Push backtest results to Google Sheets
====================================================
...
SETUP (one-time):
  ...
  3. Create a Service Account → generate JSON key → save as:
       scripts/Adaptive Pullback Momentum v2/service_account.json
...
TRADES_SHEET    = "Trades v2"      # tab for v2 trade-level data
ALERTS_SHEET    = "Alerts v2"      # tab for v2 alert events
SUMMARY_SHEET   = "Summary v2"     # tab for v2 run-level summaries
...
def push_results(trades, alerts, symbol, interval, period,
    ...
    """
    Called from backtest_apm_v2_30m.py after the simulation loop.
...
if __name__ == "__main__":
    ...
    CSV_FILE = HERE / "apm_v2_trades_btcusd_30m.csv"
    TXT_FILE = HERE / "apm_v2_alerts_btcusd_30m.txt"

    if not CSV_FILE.exists():
        print(f"Run backtest_apm_v2_30m.py first to generate {CSV_FILE.name}")
        sys.exit(1)
