"""
sweep_apm_v5_1h_s4.py — APM v5 CLM 1h   Stage-4: Micro-sweep SL/Trail
=======================================================================
Winner from S3: ADX=33, SL=1.2, TP=2.0, TRAIL=3.0 → +11.87% MDD -2.33%
This sweep:
  - Pushes SL even tighter (0.5-1.3×) around ADX=33
  - Tests TRAIL_DIST  (0.2-1.0) to let winners run longer
  - Tests slightly relaxed ADX (30, 33, 35, 38)
  - Fixed: TP=2.0, TRAIL_ACT=3.0, PB=0.20, VOL=1.2, MIN_BODY=0.05, DIRS=both

Also tries TP=3.0/4.0 with trail_act=2.0 at tight SL to see if bigger trail windows win.

Full axes:
  ADX       : 28, 30, 33, 35, 38
  SL_MULT   : 0.5, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.5
  TRAIL_DIST: 0.2, 0.3, 0.5, 0.7, 1.0
  TP_MULT   : 2.0, 3.0, 4.0  (pairs with TRAIL_ACT below)
  TRAIL_ACT : 2.0, 3.0       (only test active trail; no 99=off here)

Total: 5×8×5×3×2 = 1200 combos
Filter: n>=8, WR>=50%, MDD>-5%
Rank: net_pct
"""
...
ADX_THRS    = [28, 30, 33, 35, 38]
SL_MULTS    = [0.5, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.5]
TRAIL_DISTS = [0.2, 0.3, 0.5, 0.7, 1.0]
TP_MULTS    = [2.0, 3.0, 4.0]
TRAIL_ACTS  = [2.0, 3.0]

combos = list(itertools.product(ADX_THRS, SL_MULTS, TRAIL_DISTS, TP_MULTS, TRAIL_ACTS))
total  = len(combos)
...
for ci, (adx_thr, sl_m, tr_dist, tp_m, tr_act) in enumerate(combos):
        ...
        if in_t:
            if d == "long":
                ...
                if best >= ep + entry_atr * tr_act:
                    new_sl = max(sl, best - entry_atr * tr_dist)
                ...
                if best <= ep - entry_atr * tr_act:
                    new_sl = min(sl, best + entry_atr * tr_dist)
                    if new_sl < sl: sl = new_sl
                hit_sl = h[i] >= sl
    ...
    if total_t < 8 or wr < 50 or mdd < -5:
        continue

    calmar = round(net_pct / abs(mdd), 2) if mdd != 0 else 0
    results.append({
        "adx": adx_thr, "sl_m": sl_m, "trail_dist": tr_dist,
        "tp_m": tp_m, "trail_act": tr_act,
...
rdf.to_csv("sweep_apm_v5_1h_s4.csv", index=False)
print("\nSaved: sweep_apm_v5_1h_s4.csv")
