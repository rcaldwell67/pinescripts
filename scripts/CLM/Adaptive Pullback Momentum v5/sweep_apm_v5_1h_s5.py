"""
sweep_apm_v5_1h_s5.py — APM v5 CLM 1h  Stage-5: realistic next-bar trail sweep
=================================================================================
Fix: trail stop is applied on the NEXT bar (not same-bar), matching real execution.
     Entry at close, stop effective next bar.

Key principle:
  With TP_MULT >> TRAIL_ACT, the trail is the primary exit for big wins.
  TP acts as a "max exit" fallback.

Axes:
  ADX_THRESH : 28, 30, 33, 35
  SL_MULT    : 0.7, 0.9, 1.0, 1.2, 1.5
  TRAIL_ACT  : 1.5, 2.0, 2.5, 3.0
  TRAIL_DIST : 0.2, 0.3, 0.5, 0.7, 1.0
  TP_MULT    : 10.0  (effectively off; trail is primary exit)
  Fixed: PB=0.20, VOL=1.2, MIN_BODY=0.05, DIRS=both, RSI=20, EMA=21/34/200

Total: 4×5×4×5 = 400 combos (fast)
...
ADX_THRS    = [28, 30, 33, 35]
SL_MULTS    = [0.7, 0.9, 1.0, 1.2, 1.5]
TRAIL_ACTS  = [1.5, 2.0, 2.5, 3.0]
TRAIL_DISTS = [0.2, 0.3, 0.5, 0.7, 1.0]

combos = list(itertools.product(ADX_THRS, SL_MULTS, TRAIL_ACTS, TRAIL_DISTS))
total  = len(combos)
...
for ci, (adx_thr, sl_m, tr_act, tr_dist) in enumerate(combos):
    if ci % 80 == 0: print(f"  {ci}/{total} …", flush=True)
    ...
    pending_sl = None   # next-bar trail update
    ...
    for i in range(WARMUP, len(df)):
        ...
        if in_t:
            ...
            if d == "long":
                ...
                # Queue trail update for next bar
                if best >= ep + entry_atr * tr_act:
                    new_sl = best - entry_atr * tr_dist
                ...
                if hit_sl:
                    ...
                    sl_exits += 1 if pnl < 0 else 0
                    trail_exits += 1 if pnl >= 0 else 0
                elif hit_tp:
                    xp_ = max(o[i], tp)
                    pnl = (xp_ - ep) * qty - (ep + xp_) * qty * COMM
                    tp_exits += 1
    ...
    results.append({
        "adx": adx_thr, "sl_m": sl_m, "trail_act": tr_act, "trail_dist": tr_dist,
...
rdf.to_csv("sweep_apm_v5_1h_s5.csv", index=False)
print("\nSaved: sweep_apm_v5_1h_s5.csv")
