"""
APM v3.1  — Push backtest results to Google Sheets
====================================================
...
TRADES_SHEET    = "Trades v3"      # tab for v3 trade-level data
ALERTS_SHEET    = "Alerts v3"      # tab for v3 alert events
SUMMARY_SHEET   = "Summary v3"     # tab for v3 run-level summaries
...
def push_results(trades, alerts, symbol, interval, period,
    ...
    """
    Called from backtest_apm_v3_1h.py after the simulation loop.
...
if __name__ == "__main__":
    ...
    CSV_FILE = HERE / "apm_v3_trades_btcusd_1h.csv"
    TXT_FILE = HERE / "apm_v3_alerts_btcusd_1h.txt"

    if not CSV_FILE.exists():
        print(f"Run backtest_apm_v3_1h.py first to generate {CSV_FILE.name}")
        sys.exit(1)
