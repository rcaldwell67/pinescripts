"""
paper_trader_clm_v3.py — APM v3.4 paper trading bot, CLM 15m shorts.

Run every 15 minutes at bar close via GitHub Actions.
Uses Alpaca paper trading API.

State is persisted in docs/data/clm/v3_paper_state.json between runs so
trailing-stop tracking survives across invocations.

Completed trades are appended to docs/data/clm/v3_trades_paper.csv —
the same format the dashboard expects.

Run manually:
    cd /workspaces/pinescripts
    python "scripts/CLM/Adaptive Pullback Momentum v3/paper_trader_clm_v3.py"
...
log = logging.getLogger("apm_paper")

# ── Paths ─────────────────────────────────────────────────────────────────────
_WS         = Path(__file__).resolve().parent.parent.parent.parent
_DOCS       = _WS / "docs"
STATE_FILE  = _DOCS / "data" / "clm" / "v3_paper_state.json"
TRADES_FILE = _DOCS / "data" / "clm" / "v3_trades_paper.csv"
TRADES_COLS = [
...
# ── APM v3.4 parameters (CLM 15m — matches backtest_apm_v3_15m.py exactly) ───
SYMBOL     = "CLM"
...
PANIC_MULT = 1.5
RSI_LO_S   = 32;   RSI_HI_S = 58   # short entry bounds
SL_MULT    = 1.5
TP_MULT    = 6.0
TRAIL_ACT  = 2.5    # trail activates once price moves ATR×2.5 in favour
TRAIL_DIST = 0.4    # trail stays ATR×0.4 from best price
RISK_PCT   = 0.02   # 2% equity risked per trade
COMMISSION_PCT  = 0.0006
INITIAL_CAPITAL = 10_000.0
MIN_BARS   = EMA_SLOW + 60   # minimum bars needed for indicator warmup
...
def fetch_bars(data_client) -> pd.DataFrame:
    """Fetch 30 days of CLM 15m bars from Alpaca (ample history for indicators)."""
...
def check_short_signal(df: pd.DataFrame) -> dict | None:
    """
    Evaluate APM v3.4 short entry conditions on the most recent completed bar.
    The last row (iloc[-1]) is the completed "recovery" bar.
    The second-to-last row (iloc[-2]) is the "pullback" bar.
    ...
    # EMA21 slope: current bar vs 3 bars ago (index -4)
    if float(bar["EMA_FAST"]) >= float(df.iloc[-4]["EMA_FAST"]):
    ...
    # Pullback: prev bar's high ≥ EMA21_prev × (1 − PB_PCT%)
    ...
    # Recovery bar: close below EMA21, bearish candle
    if not (close < float(bar["EMA_FAST"]) and close < float(bar["Open"])):
        log.debug("Recovery bar not bearish — skip")
        return None
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    log.info("=== APM v3.4 Paper Trader — %s 15m ===", SYMBOL)
    ...
    log.info("Fetching %s 15m bars (30d)…", SYMBOL)
            ...
            if new_sl < pos["sl"]:   # only tighten
                ...
                try:
                    new_sl_ord = trading_client.submit_order(StopOrderRequest(
                        ...
                        client_order_id=f"apm_sl_{int(datetime.now(timezone.utc).timestamp())}",
                    ))
    ...
    try:
        trading_client.submit_order(MarketOrderRequest(
            ...
            client_order_id=f"apm_entry_{int(datetime.now(timezone.utc).timestamp())}",
        ))
    ...
    try:
        tp_ord = trading_client.submit_order(LimitOrderRequest(
            ...
            client_order_id=f"apm_tp_{int(datetime.now(timezone.utc).timestamp())}",
        ))
