# ─────────────────────────────────────────────────────────────────────────────
# APM v4  —  v4 indicator iteration  (30m BTC-USD)
#
# BASELINE: v4.1 (previous session best: +9.48%, PF=2.297, WR=70%, 20 trades)
#
...
# v4.1 confirmed best params (baseline)
PB_PCT     = 0.15
VOL_MULT   = 1.2
MIN_BODY   = 0.20
...
# ─── v4.1 baseline filters (already confirmed) ─────────────────────────────────
...
rsi_rising1    = df["RSI"] > df["RSI"].shift(1)     # single-bar (v4.1)
...
# v4.1 baseline signal
long_v41 = (long_pb & ema_bull_full & ema_slope_up & rsi_rising1 &
            rsi_long_ok & vol_ok & is_trending & ~is_panic & atr_pct_ok)
short_v41= (short_pb & ema_bear_full & ema_slope_down & rsi_falling1 &
...
# ─── v4 combined signal ─────────────────────────────────────────────────────────
long_v4 = (long_pb & ema_bull_full & ema_slope_up &
           di_bull & strong_close_l & rsi_rising2 &
           rsi_long_ok & vol_ok & is_trending & ~is_panic & atr_pct_ok)
short_v4= (short_pb & ema_bear_full & ema_slope_down &
...
print("--- v4 Signal filter pass-through (long) ---")
cumulative = pd.Series([True]*len(df), index=df.index)
for name, mask in components_v4_long:
    cumulative = cumulative & mask
    print(f"  {name:<20} → {cumulative.sum():>4} rows pass")
print("--- v4 Signal filter pass-through (short) ---")
cumulative = pd.Series([True]*len(df), index=df.index)
for name, mask in components_v4_short:
    cumulative = cumulative & mask
    print(f"  {name:<20} → {cumulative.sum():>4} rows pass")
print(f"\nv4 Signals — Long: {long_v4.sum()}  Short: {short_v4.sum()}")
...
# ─── Head-to-head: v4.1 vs v4 at confirmed best params ──────────────────────────
print("\n" + "="*60)
print("  HEAD-TO-HEAD: v4.1  vs  v4  (TP×3.0 SL×2.0 ATRf=0.20%)")
print("="*60)
t21, eq21, _ = run_sim(df, long_v41, short_v41, SL_MULT, TP_MULT, TRAIL_ACT, TRAIL_DIST, af=ATR_FLOOR)
r21 = stats(t21, eq21, f"v4.1 baseline (TP×{TP_MULT} SL×{SL_MULT})")

t3, eq3, _ = run_sim(df, long_v4, short_v4, SL_MULT, TP_MULT, TRAIL_ACT, TRAIL_DIST, af=ATR_FLOOR)
r3 = stats(t3, eq3, f"v4 new filters (TP×{TP_MULT} SL×{SL_MULT})")
...
candidates = [
    ("v4.1 baseline",          long_v41, short_v41),
    ("+ DI alignment",
        long_v41  & di_bull,
        short_v41 & di_bear),
    ("+ strong close",
        long_v41  & strong_close_l,
        short_v41 & strong_close_s),
    ("+ RSI 2-bar",
        long_v41  & rsi_rising2,
        short_v41 & rsi_falling2),
    ("+ all three (v4)",        long_v4, short_v4),
    ("+ DI + strong close",
        long_v41  & di_bull & strong_close_l,
        short_v41 & di_bear & strong_close_s),
    ("+ DI + RSI 2-bar",
        long_v41  & di_bull & rsi_rising2,
        short_v41 & di_bear & rsi_falling2),
    ("+ strong close + RSI 2",
        long_v41  & strong_close_l & rsi_rising2,
        short_v41 & strong_close_s & rsi_falling2),
]
...
# ─── Full sweep with best v4 signal set ─────────────────────────────────────────
print("\n=== Full sweep: best v4 filter combo × TP × SL × ADX ===")
...
best_iso = max(iso_rows, key=lambda x: x["ret"]) if iso_rows else {"ls": long_v4, "ss": short_v4}
...
for adx_t in adx_vals:
    ...
    if best_iso["label"] == "v4.1 baseline":
        ...
        ss_sw = (short_pb & ema_bear_full & ema_slope_down & strong_close_s & rsi_falling2 &
                 rsi_short_ok & vol_ok & is_trend_sw & ~is_panic & atr_pct_ok)
    elif "all three" in best_iso["label"] or "v4" in best_iso["label"]:
        ls_sw = (long_pb & ema_bull_full & ema_slope_up & di_bull & strong_close_l &
        ...
        # default fallback: v4 full
        ls_sw = long_v4 & is_trend_sw & ~is_panic  # rough re-filter (ADX already in v4)
        ss_sw = short_v4 & is_trend_sw & ~is_panic
    ...
    for tp_m in tp_vals:
        for sl_m in sl_vals:
            ...
            sweep_rows.append({"adx":adx_t,"tp":tp_m,"sl":sl_m,
                               ...
                               "ls":ls_sw,"ss":ss_sw})
            marker = " ←" if ret_ >= 9.48 else ""  # v4.1 best was 9.48
...
if sweep_rows:
    ...
    r_f = stats(t_f, eq_f, f"APM v4 Final  ADX>{b['adx']} TP×{b['tp']} SL×{b['sl']}")

    if r_f is not None:
        ret_f, pf_f, wr_f, mdd_f = r_f

        # save outputs
        out_csv = f"apm_v4_trades_{TICKER.replace('-','').lower()}_{INTERVAL}.csv"
        ...
        fig.autofmt_xdate()
        plt.tight_layout()
        out_png = f"apm_v4_equity_{TICKER.replace('-','').lower()}_{INTERVAL}.png"
