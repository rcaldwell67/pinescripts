# ─────────────────────────────────────────────────────────────────────────────
# APM v4  —  Indicator improvement study  (30m BTC-USD)
#
# CHANGES UNDER TEST vs v4.0 Pine Script defaults
...
def print_stats(tdf, final_eq, label=""):
    ...
    print()
    print("=" * 60)
    print(f"  {label or 'APM v4 Improved'}  —  {TICKER} {INTERVAL}")
    print("=" * 60)
...
ret_i, pf_i, wr_i, mdd_i = print_stats(tdf, final_eq, "APM v4 Improved")
...
print_stats(tdf_o, feq_o, "APM v4.0 Original (baseline)")
...
if sweep_rows:
    ...
    ret_f, pf_f, wr_f, mdd_f = print_stats(tdf_f, feq_f,
        f"APM v4 Final  (TP×{b['TP']} SL×{b['SL']} ATRf={b['ATRf']:.2f}%)")

    # save CSV + chart for final config
    out_csv = f"apm_v4_improved_trades_{TICKER.replace('-','').lower()}_{INTERVAL}.csv"
    ...
    ax1.set_title(
        f"APM v4 Improved  |  {TICKER} {INTERVAL}  |  "
    ...
    plt.tight_layout()
    out_png = f"apm_v4_improved_equity_{TICKER.replace('-','').lower()}_{INTERVAL}.png"
