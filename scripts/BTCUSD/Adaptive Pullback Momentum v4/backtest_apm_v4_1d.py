...
# ── Strategy defaults (v4.1 sweep-optimised) ────────────────────────────────
EMA_FAST_LEN = 21
EMA_MID_LEN  = 50
EMA_SLOW_LEN = 200
ADX_LEN      = 14
...
def entry_alert(direction, ts, cl, av, sd, qty, equity_at_entry, row):
    ...
    return (
        f"APM v4.1 | {direction.upper()} ENTRY | BTC-USD [1D]\n"
...
def trail_alert(direction, ts, best_px, entry_px, new_sl, target, av, entry_atr):
    ...
    return (
        f"APM v4.1 | TRAIL STOP ACTIVATED | BTC-USD [1D]\n"
...
def exit_alert(direction, ts, entry_px, exit_px, net_pnl, comm, max_runup, bars, equity_after):
    ...
    return (
        f"APM v4.1 | {direction.upper()} EXIT [{result}] | BTC-USD [1D]\n"
...
def panic_alert(started, av, atr_bl_v, ts):
    ...
    return (
        f"APM v4.1 | PANIC REGIME {status} | BTC-USD [1D]\n"
...
with open("apm_v4_alerts_btcusd_1d.txt", "w") as f:
    f.write(alert_log)
...
print(f"Alerts log  → apm_v4_alerts_btcusd_1d.txt")
