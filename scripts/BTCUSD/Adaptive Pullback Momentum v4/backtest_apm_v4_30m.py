# ─────────────────────────────────────────────────────────────────────────────
# APM v4.2 — Python backtest  (BTC-USD 30m, both directions)
#
# v4.2 changes vs v4.0 baseline:
...
#   TP mult         : 3.5x  (was 2.0x)  ← v4.2 sweep-peak
...
# ─── Signal construction (v4.2 — full Pine Script filter set) ─────────────────
...
def entry_alert(direction, ts, cl, av, atr_bl_v, sd, qty, equity_at_entry, row):
    ...
    return _al([
        f"APM v4.2 | {dl} ENTRY | {TICKER} [{INTERVAL}]",
...
def trail_alert(direction, best, entry, new_sl, old_sl, tp, av, ts):
    ...
    return _al([
        f"APM v4.2 | TRAIL STOP ACTIVATED | {TICKER} [{INTERVAL}]",
        f"Direction : {dl}",
...
def exit_alert(direction, ep, xp, pnl_dollar, comm_dollar, max_runup,
    ...
    ms   = "+" if mv >= 0 else ""
    return _al([
        f"APM v4.2 | {dl} EXIT [{res}] | {TICKER} [{INTERVAL}]",
...
def panic_alert(started, atr_v, atr_bl_v, ts):
    ...
    return _al([
        f"APM v4.2 | {lbl} | {TICKER} [{INTERVAL}]",
...
if tdf.empty:
    ...
    print()
    print("=" * 60)
    print(f"  APM v4.2  —  {TICKER} {INTERVAL}  (max)")
    print("=" * 60)
...
alert_out   = f"apm_v4_alerts_{TICKER.replace('-','').lower()}_{INTERVAL}.txt"
...
if not tdf.empty:
    ...
    ax1.set_title(f"APM v4.2  |  {TICKER} {INTERVAL}  |  "
