...
# ─── Indicators (full history for proper EMA/ADX warmup) ──────────────────────
...
atr_fl      = (df["ATR"] / df["Close"] >= ATR_FLOOR) if ATR_FLOOR > 0 else pd.Series(True, index=df.index)
...
print(f"\nSimulation window: {df.index[0].date()} → {df.index[-1].date()}  ({len(df)} bars)")
...
for ts, row in df.iterrows():
    ...
    if pos is not None:
        ...
        if pos["direction"] == "short":
            if low < pos["best"]:
                pos["best"] = low
            if pos["best"] <= pos["entry"] - atr * TRAIL_ACT:
                new_sl = pos["best"] + atr * TRAIL_DIST
                if new_sl < pos["sl"]:
                    pos["sl"] = new_sl
            if MAX_BARS > 0 and bars_in_trade >= MAX_BARS:
                ...
                trades.append({"entry_time": pos["entry_time"], "exit_time": ts,
                               ...
                               "result": "MB", "pnl_pct": round(pnl_raw * 100, 3),
            ...
            if hit_tp or hit_sl:
                ...
                trades.append({"entry_time": pos["entry_time"], "exit_time": ts,
                               "direction": "short", "entry": pos["entry"], "exit": xp,
                               "result": result, "pnl_pct": round(pnl_raw * 100, 3),
                               "dollar_pnl": round(dp, 2), "equity": round(equity, 2)})
                pos = None; bars_in_trade = 0
...
monthly["wr_pct"] = monthly["wins"] / monthly["trades"] * 100
monthly["cum_pnl"] = monthly["net_pnl"].cumsum()
print("\n─── Monthly Breakdown ───")
print(f"{'Month':<9}  {'Trades':>6}  {'WR%':>6}  {'Net $':>8}  {'Cum $':>9}")
print("─" * 46)
for _, r in monthly.iterrows():
    flag = "▲" if r["net_pnl"] > 0 else "▼"
    print(f"{str(r['month']):<9}  {r['trades']:>6.0f}  {r['wr_pct']:>6.1f}%  "
          f"{r['net_pnl']:>+8.2f}  {r['cum_pnl']:>+9.2f}  {flag}")
...
# ─── Save trade log CSV ────────────────────────────────────────────────────────
out_csv = "apm_v1_12mo_trades_btcusd_5m.csv"
...
_twelve_month_out = _Path(__file__).resolve().parent.parent.parent.parent / "docs" / "data" / "btcusd" / "v1_trades_12mo.csv"
tdf.rename(columns={"entry": "entry_price", "exit": "exit_price"})[
    ["entry_time", "exit_time", "direction", "entry_price", "exit_price",
     "result", "pnl_pct", "dollar_pnl", "equity"]
].to_csv(_twelve_month_out, index=False)
...
fig.suptitle(f"APM v1.1 · 5m  ·  BTCUSD  ·  12-Month  |"
...
for _, t in tdf.iterrows():
    ...
    ax1.axvline(pd.Timestamp(str(t["exit_time"])), color=col, linewidth=0.5, alpha=0.4)
...
plt.tight_layout()
out_png = "apm_v1_12mo_equity_btcusd_5m.png"
