"""
APM v1.3  — Push backtest results to Google Sheets
====================================================
...
SETUP (one-time):
  ...
  3. Create a Service Account → generate JSON key → save as:
       scripts/Adaptive Pullback Momentum v1/service_account.json
...
# ── Parsers for the structured alert list produced by backtest_apm_v1.py ───────
...
def push_results(trades, alerts, symbol, interval, period,
    ...
    """
    Call this from backtest_apm_v1.py after the simulation loop.
...
if __name__ == "__main__":
    ...
    CSV_FILE = HERE / "apm_v1_trades_btcusd_15m.csv"
    TXT_FILE = HERE / "apm_v1_alerts_btcusd_15m.txt"

    if not CSV_FILE.exists():
        print(f"Run backtest_apm_v1.py first to generate {CSV_FILE.name}")
        sys.exit(1)
