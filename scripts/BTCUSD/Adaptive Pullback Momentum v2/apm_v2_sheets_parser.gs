...
function parseAlertBody(body, date, msgId) {
  ...
  if (header.indexOf("APM v2.2") === -1) return null;
  ...
  // Header format: "APM v2.2 | SHORT ENTRY | BTC-USD [30m]"
...
function testParse() {
  var threads = GmailApp.search('from:noreply@tradingview.com subject:"APM v2.2"', 0, 3);
  for (var i = 0; i < threads.length; i++) {
    var msgs = threads[i].getMessages();
    for (var j = 0; j < msgs.length; j++) {
      var msg = msgs[j];
