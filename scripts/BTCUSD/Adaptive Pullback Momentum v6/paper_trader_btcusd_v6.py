"""
paper_trader_btcusd_v6.py — APM v6.1 paper trading bot, BTC/USD 1D (longs + shorts).

Run once per day at/after daily bar close via GitHub Actions (or cron). Evaluates
APM v6.1 long and short entry conditions and manages any open position (trailing stop).

Strategy parameters (Pine v6.1 1D, sweep-optimised):
    ADX=28 | PB=0.15% | SL×2.0 | TP×1.0 | TRAIL_ACT=2.0× | TRAIL_DIST=0.4×
    PANIC=2.0× | VOL=1.0× | MIN_BODY=0.30× | Both longs + shorts

State:  docs/data/btcusd/v6_paper_state.json
Trades: docs/data/btcusd/v6_trades_paper.csv

Run manually:
    cd /workspaces/pinescripts
    python "scripts/BTCUSD/Adaptive Pullback Momentum v6/paper_trader_btcusd_v6.py"
...
log = logging.getLogger("apm_btcusd_v6")

# ── Paths ─────────────────────────────────────────────────────────────────────
_WS         = Path(__file__).resolve().parent.parent.parent.parent
STATE_FILE  = _WS / "docs" / "data" / "btcusd" / "v6_paper_state.json"
TRADES_FILE = _WS / "docs" / "data" / "btcusd" / "v6_trades_paper.csv"
TRADES_COLS = [
...
# ── APM v6.1 parameters (BTC/USD 1D — +20% sizing calibration) ───────────────
SYMBOL          = "BTC/USD"
...
def fetch_bars(data_client) -> pd.DataFrame:
    """Fetch 400 days of BTC/USD 1D bars for indicator warmup."""
...
def check_signal(df: pd.DataFrame) -> dict | None:
    """Evaluate APM v6.1 entry (long or short) on the most recent completed 1D bar."""
    if len(df) < 5:
        return None
    ...
    if TRADE_LONGS:
        ...
        if ema_bull and long_pb and rsi_ok_l:
            sl = close - stop_dist
    ...
    if TRADE_SHORTS:
        ...
        if ema_bear and short_pb and rsi_ok_s:
            sl = close + stop_dist
            tp = close - atr * TP_MULT
            log.info(
...
def submit_entry(tc, direction: str, qty: float) -> None:
    ...
    tc.submit_order(MarketOrderRequest(
        ...
        client_order_id=f"apm_btc_v6_entry_{int(datetime.now(timezone.utc).timestamp())}",
    ))
...
def submit_sl(tc, direction: str, qty: float, sl_price: float) -> str:
    ...
    o = tc.submit_order(StopOrderRequest(
        ...
        client_order_id=f"apm_btc_v6_sl_{int(datetime.now(timezone.utc).timestamp())}",
    ))
    return str(o.id)
...
def submit_tp(tc, direction: str, qty: float, tp_price: float) -> str:
    ...
    o = tc.submit_order(LimitOrderRequest(
        ...
        client_order_id=f"apm_btc_v6_tp_{int(datetime.now(timezone.utc).timestamp())}",
    ))
    return str(o.id)
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    log.info("=== APM v6.1 Paper Trader — %s 1D (longs + shorts) ===", SYMBOL)
