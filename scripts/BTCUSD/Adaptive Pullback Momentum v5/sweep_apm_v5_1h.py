"""
Multi-phase improvement sweep — APM v5.0 @ 1h BTC-USD
Builds on each phase's best result to find optimal param set.
"""
...
# ── Download once ──────────────────────────────────────────────────────────────
print("Downloading BTC-USD 1h max …")
raw = yf.download("BTC-USD", period="max", interval="1h",
...
def backtest(cfg):
    ADX_THRESH  = cfg.get("adx", 25.0)
    PB_PCT      = cfg.get("pb_pct", 0.15)
    VOL_MULT    = cfg.get("vol_mult", 1.0)
    MIN_BODY    = cfg.get("min_body", 0.15)
    PANIC_MULT  = cfg.get("panic", 1.5)
    SL_MULT     = cfg.get("sl", 1.5)
    TP_MULT     = cfg.get("tp", 2.0)
    TRAIL_ACT   = cfg.get("trail_act", 1.5)
    TRAIL_DIST  = cfg.get("trail_dist", 0.8)
    ATR_FLOOR   = cfg.get("atr_floor", 0.0)
    RSI_LO_L    = cfg.get("rsi_lo_l", 42.0); RSI_HI_L = cfg.get("rsi_hi_l", 68.0)
    RSI_LO_S    = cfg.get("rsi_lo_s", 32.0); RSI_HI_S = cfg.get("rsi_hi_s", 58.0)
    ...
    l_sig = long_sig.values; s_sig = short_sig.values
    ...
    eq_ext = np.concatenate([[INIT_CAP], eq_a])
    dd = ((eq_ext - rm)/rm*100).min()
    return {"pct": pct, "pf": pf, "wr": wr, "n": n, "dd": dd,
            "nl": len(t[t["dir"]=="long"]), "ns": len(t[t["dir"]=="short"]),
            "wrl": (t[t["dir"]=="long"]["pnl"]>0).mean()*100 if len(t[t["dir"]=="long"])>0 else 0,
            "wrs": (t[t["dir"]=="short"]["pnl"]>0).mean()*100 if len(t[t["dir"]=="short"])>0 else 0}
...
for ta in [1.0, 1.5, 2.0, 2.5]:
    for td in [0.5, 0.6, 0.8, 1.0, 1.2]:
        cfg = {**p5_base, "trail_act": ta, "trail_dist": td}
        r = backtest(cfg)
        row(cfg, r, f"trail act×{ta}  dist×{td}")
...
# ───────────────────────────────────────────────────────────────────────────────
# PHASE 7 — Long / short direction
# ───────────────────────────────────────────────────────────────────────────────
hdr("PHASE 7 — Direction: longs / shorts / both")
p7_base = {**ph6_best[0]}
ph7_best = None
for lg, sh in [(True,True),(True,False),(False,True)]:
    cfg = {**p7_base, "longs": lg, "shorts": sh}
    r = backtest(cfg)
    tag = "both" if lg and sh else ("longs only" if lg else "shorts only")
    row(cfg, r, tag)
    if ph7_best is None or r["pct"] > ph7_best[1]["pct"]: ph7_best = (cfg, r)
print(f"\n  ★ Best: longs={ph7_best[0]['longs']} "
      f"shorts={ph7_best[0]['shorts']}  → {ph7_best[1]['pct']:+.2f}%")

# ───────────────────────────────────────────────────────────────────────────────
# FINAL SUMMARY
# ───────────────────────────────────────────────────────────────────────────────
best_cfg, best_r = ph7_best
print(f"\n{'='*60}")
print(f"  OPTIMAL CONFIGURATION  ({best_r['pct']:+.2f}% net)")
print(f"{'='*60}")
