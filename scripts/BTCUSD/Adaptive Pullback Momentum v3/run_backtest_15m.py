...
# Signals (v3.1 defaults later in script); initial v3.0 signals
tol = PB_PCT / 100.0
long_pb  = (
...
# Bar-by-bar backtest (v3.0 defaults)
...
if equity_curve:
    ...
    ax.set_title(f"APM v3.0 Equity — {TICKER} {INTERVAL}")
...
# ------------------------- v3.1 simulation (updated defaults) -------------------------
print("\nRunning APM v3.1 simulation on same data...")
...
tol11 = V11_PB / 100.0
body_v31 = (df["Close"] - df["Open"]).abs() / df["ATR"] >= V11_BODY

long_pb11 = (
    (df["Low"].shift(1) <= df["EMA_FAST"].shift(1) * (1 + tol11)) &
    (df["Close"] > df["EMA_FAST"]) &
    (df["Close"] > df["Open"]) & body_v31
)
short_pb11 = (
...
print(f"v3.1 Signals — Long: {long_sig11.sum()}  Short: {short_sig11.sum()}")
...
if not t11df.empty:
    ...
    ax.axhline(INITIAL_CAPITAL, color="white", ls=":")
    ax.set_title(f"APM v3.1 Equity — {TICKER} {INTERVAL}")
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %d"))
    plt.tight_layout()
    out_png = f"backtest_apm_v31_{TICKER.replace('-','').lower()}_{INTERVAL}.png"
    plt.savefig(out_png, dpi=150, bbox_inches="tight")
    print(f"v3.1 Chart saved → {out_png}")
    # save trades
    out_csv = f"backtest_apm_v31_trades_{TICKER.replace('-','').lower()}_{INTERVAL}.csv"
    t11df.to_csv(out_csv, index=False)
    print(f"v3.1 Trades CSV saved → {out_csv}")
else:
    print("v3.1 produced no trades.")
...
proposed_cfg = {
    ...
    "TrailDist": 1.0,  # trail distance (looser than v31)
